...
const bookDecoder = {
  ...
  function createButton(verseNumber, verseNumberText, scriptureCount, savedColor, scripturePath) {
    ...
    chrome.storage.sync.get('apostleOnly', function (data) {
      ...
      let scriptureQuotedDataUrl = 'https://www.kameronyork.com/datasets/all-footnotes-oct-2024.json';
      if (apostleOnly) {
        scriptureQuotedDataUrl = 'https://www.kameronyork.com/datasets/apostle-all-footnotes-oct-2024.json';
      }
  ...
  async function replaceVerseNumbersWithButtons(callback) {
    ...
    if (langRegex.test(url)) {
      const promises = Array.from(verseNumbers).map(async (verseNumber) => {
        ...
        let scriptureQuotedDataUrl = 'https://www.kameronyork.com/datasets/all-footnotes-lookup-oct-2024.json';
        if (apostleOnly) {
          scriptureQuotedDataUrl = 'https://www.kameronyork.com/datasets/apostle-all-footnotes-lookup-oct-2024.json';
        }
