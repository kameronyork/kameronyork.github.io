...
# -------------------------------------------------
# Manual release date overrides for weekend titles
# Format: IMDB_ID: "YYYY-MM-DD"
# -------------------------------------------------
WEEKEND_RELEASE_DATES = {
    "rl2127396865": "2025-09-13",
}
...
def get_weekend_year(imdb_id):
    """
    Returns the year to use for weekend date parsing.
    Priority:
    1. Manual override
    2. Current year fallback
    """
    if imdb_id in WEEKEND_RELEASE_DATES:
        return datetime.strptime(
            WEEKEND_RELEASE_DATES[imdb_id], "%Y-%m-%d"
        ).year
...
def fetch_weekend_data(imdb_id, title):
    ...
    release_year = get_weekend_year(imdb_id)
    ...
    for row in rows[1:]:
        ...
        try:
            ...
            start_date = datetime.strptime(
                f"{start_str} {release_year}", "%b %d %Y"
            )

            # Handle year rollover (Dec → Jan)
            if release_date and start_date < release_date:
                start_date = start_date.replace(year=release_year + 1)

            try:
                end_date = datetime.strptime(
                    f"{end_str} {start_date.year}", "%b %d %Y"
                )
            except ValueError:
                end_day = int(end_str)
                end_date = datetime(
...
for person in people_movies:
    for movie in person["movies"]:
        ...
        if pull_through:
            try:
                pull_through_date = datetime.strptime(
                    pull_through, "%Y-%m-%d"
                ).date()
...
try:
    ...
    subprocess.run(
        ['git', 'commit', '-m',
         'Update box office data [skip ci]'],
        check=True
    )
