...
def fetch_weekend_data(imdb_id, title):
    ...
    # Get release year and month from the mapping
    release_date_str = MOVIE_RELEASE_DATES.get(imdb_id)
    if release_date_str:
        release_date_obj = datetime.strptime(release_date_str, "%Y-%m-%d")
        base_year = release_date_obj.year
        release_month = release_date_obj.month
    else:
        # Fallback to current year if ID not in list
    ...
    for row in rows[1:]:
        ...
        try:
            ...
            # Determine the month of this specific weekend
            start_month_str = start_str.split(' ')[0]
            start_month_val = datetime.strptime(start_month_str, "%b").month
            
            # If the weekend month is earlier than the release month, it's the next year
            if start_month_val < release_month:
            ...
            try:
                ...
                # Handle cases like "Dec 31 - Jan 2"
                end_day = int(end_str)
