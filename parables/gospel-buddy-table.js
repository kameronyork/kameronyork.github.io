function createTableView(entries, matthewVerses, markVerses, lukeVerses, johnVerses) {
      ...
      uniqueEntriesArray.forEach(entry => {
          const maxVersion = Object.keys(entry.counts).reduce((a, b) => entry.counts[a].size > entry.counts[b].size ? a : b);
          const maxVersesQuoted = entry.counts[maxVersion].size;
